...
def main():
    ...
    while True:
        ...
        if debug==True:
            ...
            pygame.draw.circle(screen,"yellow",database["units_teamA"].sprites()[0].move_target_finish,3)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == K_ESCAPE:
                    ...
                    exit()
                if event.key == K_LEFT:
                    for tmp in database["units_teamB"].sprites()+database["units_teamA"].sprites():
                        if tmp.move_algorithm=="movemendAI_B":
                            tmp.direction = "LEFT"
                            tmp.action = "WALK"
